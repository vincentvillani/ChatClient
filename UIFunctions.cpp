...
void UIDraw(ClientData* clientdata)
{
	...
	//Plus one more row for the space between the current message and the latest messages
	rowsUsed += 1;

	//Find out where the user is currently looking in the chat message log and draw from there.
	for(uint32_t i = rowsUsed; rowsUsed < cd->maxRow; ++i)
	{
